...
def valid(epoch):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, (inputs, loc_targets, cls_targets, center_targets, _) in enumerate(valid_loader):
            inputs = inputs.to(device)
            loc_targets = loc_targets.to(device)
            cls_targets = cls_targets.to(device)
            center_targets = center_targets.to(device)
            ...
            print('[Valid] epoch: %3d | iter: %4d | loc_loss: %.3f | cls_loss: %.3f | center_loss: %.3f | '
                  'valid_loss: %.3f | avg_loss: %.3f | num_pos: %4d'
                % (epoch, batch_idx, loc_loss.item(), cls_loss.item(), center_loss.item(),
                   loss.item(), avg_valid_loss, num_pos))
            ...
            summary_writer.add_scalar('valid/center_loss', center_loss.item(), global_iter_valid)
